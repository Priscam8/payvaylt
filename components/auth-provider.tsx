...
type AuthContextValue = {
  ...
  uploadFicaDocument: (
    title: string,
    file: { uri: string; name: string; mimeType?: string | null; size?: number | null; file?: File | null }
  ) => Promise<void>;
  completeVerificationQuestions: () => Promise<void>;
...
function normalizePersistedState(input: Partial<PersistedState> | null | undefined): PersistedState {
  ...
  return {
    ...
    documentUploads: input.documentUploads ?? seeded.documentUploads,
...
function mergeCustomerDashboard(
  ...
  const documentUploads = [
    ...dashboard.documentUploads,
    ...current.documentUploads.filter((documentUpload) => documentUpload.customerId !== mergedCustomer.id),
  ];
...
export function AuthProvider({ children }: PropsWithChildren) {
  ...
  const visibleDocumentUploads = useMemo(() => {
    if (!currentCustomer) {
      return [];
    }

    return [...state.documentUploads]
      .filter((documentUpload) => documentUpload.customerId === currentCustomer.id)
      .sort((left, right) => right.uploadedAt.localeCompare(left.uploadedAt));
  }, [currentCustomer, state.documentUploads]);
  ...
  async function beginCustomerSignIn(payload: CustomerDraft) {
    try {
      ...
      if (challenge.devCode) {
        setAuthMessage(`Demo OTP for ${challenge.destination}: ${challenge.devCode}`);
      }
  ...
  async function verifyOtp(code: string) {
    ...
    if (!pendingOtp?.challengeId || code.trim().length !== 6) {
      setAuthMessage('Enter the OTP code for this sign-in or account activation request.');
      return 'invalid';
    }
  ...
  async function uploadFicaDocument(
    title: string,
    file: { uri: string; name: string; mimeType?: string | null; size?: number | null; file?: File | null }
  ) {
    const current = stateRef.current;
    ...
    try {
      clearAuthMessage();
      const dashboard = await payVayltApi.uploadFicaDocument(
        customer.id,
        title,
        file,
        session.token
      ...
      setAuthMessage(`${title} uploaded successfully.`);
    } catch (error) {
      setAuthMessage(
        extractErrorMessage(error, 'The document upload could not be completed.')
      );
    }
  }

  async function completeVerificationQuestions() {
    const current = stateRef.current;
    const customer = findActiveCustomer(current);
    const session = current.session;
    ...
    try {
      clearAuthMessage();
      const dashboard = await payVayltApi.completeVerificationQuestions(customer.id, session.token);
      commitState(mergeCustomerDashboard(current, dashboard, session));
    } catch (error) {
      setAuthMessage(
        extractErrorMessage(error, 'The verification questions could not be completed.')
      );
    }
  }
  ...
  function signOut() {
    ...
    if (token) {
      void payVayltApi.signOut(token).catch(() => {
        // Best-effort invalidation for local development sessions.
      });
    }
  }
  ...
  const value: AuthContextValue = {
    ...
    verificationChecks,
    ficaDocuments,
    documentUploads: visibleDocumentUploads,
    dashboardStats,
