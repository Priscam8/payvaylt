...
const passwordResetLifetimeMs = 20 * 60 * 1000;
const devOtpPreviewEnabled = process.env.PAYVAYLT_ALLOW_DEV_CODES !== 'false';
...
function verifySecret(secret, hashedValue) {
  ...
  return crypto.timingSafeEqual(digestBuffer, derived);
}
...
function createOtpChallenge(flow, customerId, destination) {
  ...
  return {
    ...
    expiresAt: addMs(createdAt, otpLifetimeMs),
    attemptsRemaining: 5,
    channel: 'sms',
    provider: 'console',
    deliveryReference: null,
    ...(devOtpPreviewEnabled ? { devCode: code } : {}),
  };
}

function createPasswordResetChallenge(customerId, destination) {
  const createdAt = new Date();
  ...
  return {
    ...
    destination,
    createdAt: createdAt.toISOString(),
    expiresAt: addMs(createdAt, passwordResetLifetimeMs),
  };
}
...
function createCustomerSession(customer) {
  ...
  return {
    ...
    createdAt: createdAt.toISOString(),
    expiresAt: addMs(createdAt, sessionLifetimeMs),
  };
}
...
function createSeedDatabase() {
  ...
  const customer = {
    fullName: defaultCustomer.fullName,
    email: defaultCustomer.email,
    mobile: defaultCustomer.mobile,
    idNumber: defaultCustomer.idNumber,
    passwordHash: hashSecret(defaultCustomer.password),
    id: seededCustomerId,
    createdAt,
  ...
  const merchant = {
    companyName: defaultMerchant.companyName,
    workEmail: defaultMerchant.workEmail,
    vendorNames: defaultMerchant.vendorNames,
    passwordHash: hashSecret(defaultMerchant.password),
    id: seededMerchantId,
    createdAt,
  };
